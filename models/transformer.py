...
Filname: dataloader.py
Authors: ZhangAo(@baidu.com)
Date: 2021-01-22 17:26:03
"""
...
def collate_batch_data_v2(origin_batch, config):
    ...
    for orig_input, orig_label in origin_batch:
        if orig_input.value_indexes[-1] > 510:
            logging.warning('sequence is too long: %d. question is %s',
                            orig_input.value_indexes[-1] + 2, orig_input.question)
            continue
    ...
    TOKEN_IDS = pad_sequences(TOKEN_IDS, max_len=config.max_seq_len)
    SENT_IDS = pad_sequences(SENT_IDS, max_len=config.max_seq_len)

    QUESTION_TOKENS_INDEX = pad_sequences(QUESTION_TOKENS_INDEX, max_len=config.max_question_len)
    TABLE_INDEX = pad_sequences(TABLE_INDEX, max_len=config.max_table_num)
    COLUMN_INDEX = pad_sequences(COLUMN_INDEX, max_len=config.max_column_num)
    VALUE_INDEX = pad_sequences(VALUE_INDEX, max_len=config.max_column_num * 2)
