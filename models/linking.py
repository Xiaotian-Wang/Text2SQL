...
"""linking utils, including schema linking, cell value linking.
partly referenced to rat-sql.

Filname: linking_utils.py
Authors: ZhangAo(@baidu.com)
Date: 2020-07-13 15:57:21
"""
...
def compute_schema_linking(tokens, db):
    ...
    def partial_match(x_list, y_list):
        ...
        if x_str in STOPWORDS:
            return False
        if re.match(rf"{re.escape(x_str)}", y_str):
